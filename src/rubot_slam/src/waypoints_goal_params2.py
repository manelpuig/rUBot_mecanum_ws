...
def movebase_client():
    ...
    f = open("/home/user/rUBot_mecanum_ws/src/rubot_slam/config/waypoints.yaml", "r")
    ...
    for i in range(lines):
        ...
        goal = rospy.get_param(goal_name)
        goal_pose = create_pose_stamped(goal['x'], goal['y'], radians(goal['w']))
        goal_found = False
        while not goal_found:
            client.send_goal(goal_pose)
            reached = client.wait_for_result(rospy.Duration(5))
            if not reached:
                if state != actionlib.GoalStatus.ACTIVE:
                    rospy.loginfo("Correct the actual robot POSE")
                    rospy.sleep(30)
                else:
                    rospy.loginfo("Robot has not yet reached the goal-POSE")
            else:
                goal_found = True
                rospy.loginfo("Navigation is REACHED!")
